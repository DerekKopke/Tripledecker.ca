...
Weebly.mobile_navigation = (function ($) {
	...
	var menuTap = function (e) {
		...
		if (navStyle === SLIDE_DOWN) {
			...
			fromCSS = {
				"-webkit-transform":
					"translate3d( 0px, " + effectStart + "px, 0)",
			};

			toCSS = {
				"-webkit-transform":
					"translate3d( 0px, " + effectEnd + "px, 0)",
			};
			...
			cb = function () {
				...
				if (!isOpen) {
					var $backgroundImage = Weebly.mobile
						? $("#w-fixed-bg")
						: $(".wsite-background");
			...
			if (!isOpen) {
				...
				$windowProxy =
					$windowProxy.length > 0 ? $windowProxy : $(window);

				$pageWrapper.css({
					"background-color":
						$backgroundImage.css("background-color") || "white",
					"background-image":
						$backgroundImage.css("background-image"),
					"background-position": $backgroundImage.css(
						"background-position"
					),
					"background-size": $backgroundImage.css("background-size"),
					"background-repeat":
						$backgroundImage.css("background-repeat"),
					"background-clip": $backgroundImage.css("background-clip"),
					height:
						Math.max(
							$backgroundImage.height(),
							$windowProxy.height() + browserPadding
						) + "px",
				});

				//eliminate the 'flicker' from waiting for the background image to load
				if (
					$backgroundImage.css("background-image").length > 0 &&
					supports3D &&
					supportsTouch
				) {
					timeoutLength = 260;
	...
	var tweenMenu = function ($oldSlide, $newSlide, rightToLeft) {
		...
		var newHeight = Math.max(
			$newSlide.outerHeight(),
			$oldSlide.outerHeight()
		);
		...
		if (supports3D) {
			$newSlide.css({
				"-webkit-transform":
					"translate3d(" + sign * menuWidth + "px, 0px, 0)",
			});
		} else {
			$newSlide.css({
	...
	var initMenu = function () {
		...
		$sideMenus.each(function () {
			...
			if ($parentAnchor.attr("href")) {
				...
				var $rootLink = $(
					'<li class="wsite-menu-master-item"></li>'
				).html($sideMenuRoot);
			}
			...
			$parentAnchor.append(
				'<span class="wsite-menu-mobile-arrow"></span>'
			);
	...
	var initSlideRight = function () {
		...
		$body.prepend(
			'<div id="wsite-viewport" ><div id="wsite-page-wrapper" /></div>'
		);
		var $pageWrapper = $("#wsite-page-wrapper");
